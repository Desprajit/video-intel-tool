...
COMPANY_COLORS = [ACCENT, ACCENT2, ACCENT3, ACCENT4,
...
def section_header(slide, label, color=ACCENT):
    ...
    add_text(slide, label.upper(), Inches(0.6), Inches(0.28), Inches(8), Inches(0.5),
             size=Pt(9), bold=True, color=color)

def slide_title(slide, title, subtitle=None):
    add_text(slide, title, Inches(0.5), Inches(0.7), Inches(12.3), Inches(0.9),
             size=Pt(28), bold=True, color=WHITE)
    if subtitle:
        add_text(slide, subtitle, Inches(0.5), Inches(1.45), Inches(12.3), Inches(0.45),
...
def metric_card(slide, x, y, w, h, label, value, sub=None, color=ACCENT):
    ...
    add_text(slide, label, x + Inches(0.15), y + Inches(0.12), w - Inches(0.3), Inches(0.3),
             size=Pt(9), color=GRAY)
    add_text(slide, str(value), x + Inches(0.15), y + Inches(0.35), w - Inches(0.3), Inches(0.5),
             size=Pt(22), bold=True, color=color)
    if sub:
        add_text(slide, sub, x + Inches(0.15), y + Inches(0.8), w - Inches(0.3), Inches(0.25),
                 size=Pt(9), color=GRAY)
...
def slide_cover(prs, report_data):
    ...
    # decorative circles
    add_rect(slide, Inches(9), Inches(-1), Inches(5), Inches(5), fill=RGBColor(0x1A, 0x22, 0x40))
    add_rect(slide, Inches(10.5), Inches(4), Inches(4), Inches(4), fill=RGBColor(0x13, 0x1A, 0x33))

    add_text(slide, "VIDEO COMPETITOR", Inches(0.6), Inches(1.2), Inches(9), Inches(0.8),
             size=Pt(42), bold=True, color=WHITE)
    add_text(slide, "INTELLIGENCE REPORT", Inches(0.6), Inches(1.9), Inches(9), Inches(0.8),
             size=Pt(42), bold=True, color=ACCENT)

    companies = report_data.get("companies", [])
    main = report_data.get("main_company", "")
    company_names = " · ".join([c["company"] for c in companies if "error" not in c])
    add_text(slide, company_names, Inches(0.6), Inches(2.9), Inches(10), Inches(0.5),
             size=Pt(14), color=GRAY)

    add_text(slide, f"Prepared for: {main}", Inches(0.6), Inches(3.5), Inches(8), Inches(0.4),
             size=Pt(12), color=ACCENT2)
    add_text(slide, f"Report Date: {datetime.date.today().strftime('%B %d, %Y')}",
             Inches(0.6), Inches(3.95), Inches(8), Inches(0.35), size=Pt(11), color=GRAY)

    add_text(slide, "Powered by YouTube Data API + Gemini AI",
             Inches(0.6), Inches(6.8), Inches(8), Inches(0.35), size=Pt(9), color=GRAY)
...
def slide_executive_summary(prs, report_data):
    ...
    add_text(slide, summary, Inches(0.6), Inches(2.0), Inches(7.7), Inches(1.8),
             size=Pt(13), color=WHITE, wrap=True)

    # Leader badge
    add_rect(slide, Inches(9), Inches(1.9), Inches(3.9), Inches(2), fill=ACCENT, line_color=ACCENT2, line_width=Pt(1.5))
    add_text(slide, "🏆  LEADER", Inches(9.1), Inches(2.0), Inches(3.7), Inches(0.4),
             size=Pt(10), bold=True, color=ACCENT2)
    add_text(slide, leader, Inches(9.1), Inches(2.35), Inches(3.7), Inches(0.6),
             size=Pt(20), bold=True, color=WHITE)
    add_text(slide, leader_reason, Inches(9.1), Inches(2.85), Inches(3.6), Inches(0.8),
             size=Pt(10), color=WHITE, wrap=True)
    ...
    for i, comp in enumerate(companies[:5]):
        ...
        add_rect(slide, cx, Inches(4.2), Inches(2.6), Inches(1.5), fill=BG_CARD, line_color=color, line_width=Pt(1))
        add_text(slide, comp["company"][:18], cx + Inches(0.1), Inches(4.28), Inches(2.4), Inches(0.35),
                 size=Pt(10), bold=True, color=color)
        add_text(slide, fmt_num(comp["subscribers"]) + " subs",
                 cx + Inches(0.1), Inches(4.6), Inches(2.4), Inches(0.35), size=Pt(13), bold=True, color=WHITE)
        add_text(slide, f"Avg {fmt_num(comp['avg_views'])} views",
                 cx + Inches(0.1), Inches(4.9), Inches(2.4), Inches(0.3), size=Pt(10), color=GRAY)
...
def slide_channel_overview(prs, report_data):
    ...
    for j, (hdr, cx, cw) in enumerate(zip(headers, col_x, col_w)):
        add_text(slide, hdr, cx + Inches(0.05), Inches(2.0), cw, Inches(0.4),
    ...
    for i, comp in enumerate(companies[:5]):
        ...
        row_vals = [
            ...
            fmt_num(comp["subscribers"]),
            fmt_num(comp["total_videos"]),
            fmt_num(comp["total_views"]),
            comp.get("channel_since", "N/A")[:7]
        ]
        for j, (val, cx, cw) in enumerate(zip(row_vals, col_x, col_w)):
            add_text(slide, val, cx + Inches(0.1), row_y + Inches(0.15), cw - Inches(0.1), Inches(0.35),
                     size=Pt(10), color=color if j == 0 else WHITE, bold=(j==0))

    # Bar chart for subscribers
    chart_data = ChartData()
    chart_data.categories = [c["company"][:15] for c in companies[:5]]
    chart_data.add_series("Subscribers", [c["subscribers"] for c in companies[:5]])

    chart = slide.shapes.add_chart(
        XL_CHART_TYPE.COLUMN_CLUSTERED,
        Inches(0.3), Inches(5.5), Inches(12.7), Inches(1.7),
        chart_data
    ).chart
...
def slide_content_performance(prs, report_data):
    ...
    companies = [c for c in report_data.get("companies", []) if "error" not in c]
    col_w = Inches(13.33 / max(len(companies), 1))
    ...
    for i, comp in enumerate(companies[:4]):
        ...
        add_text(slide, comp["company"][:18], cx + Inches(0.1), Inches(1.92), col_w - Inches(0.25), Inches(0.3),
                 size=Pt(10), bold=True, color=WHITE)

        top5 = comp.get("top_videos", [])[:4]
        for j, vid in enumerate(top5):
            vy = Inches(2.3) + j * Inches(1.15)
            add_rect(slide, cx, vy, col_w - Inches(0.15), Inches(1.1), fill=BG_CARD, line_color=color, line_width=Pt(0.5))
            title = vid["title"][:55] + ("…" if len(vid["title"]) > 55 else "")
            add_text(slide, title, cx + Inches(0.08), vy + Inches(0.05), col_w - Inches(0.2), Inches(0.5),
                     size=Pt(8.5), color=WHITE, wrap=True)
            add_text(slide, f"👁 {fmt_num(vid['views'])}   👍 {fmt_num(vid['likes'])}   💬 {fmt_num(vid['comments'])}",
                     cx + Inches(0.08), vy + Inches(0.65), col_w - Inches(0.2), Inches(0.25),
                     size=Pt(8), color=color)
            add_text(slide, f"Engagement: {vid['engagement_rate']}%",
                     cx + Inches(0.08), vy + Inches(0.85), col_w - Inches(0.2), Inches(0.2),
...
def slide_topics_themes(prs, report_data):
    ...
    for i, comp in enumerate(companies[:5]):
        ...
        add_text(slide, comp["company"][:18], cx + Inches(0.08), Inches(1.92), col_w - Inches(0.2), Inches(0.3),
                 size=Pt(10), bold=True, color=WHITE)

        topics = themes_map.get(comp["company"], []) or comp.get("top_topics", [])[:8]
        for j, topic in enumerate(topics[:8]):
            ...
            add_text(slide, str(topic)[:35], cx + Inches(0.12), ty + Inches(0.08), col_w - Inches(0.2), Inches(0.3),
...
def slide_posting_frequency(prs, report_data):
    ...
    for i, comp in enumerate(companies[:5]):
        ...
        color = COMPANY_COLORS[i % len(COMPANY_COLORS)]
        metric_card(slide, cx, Inches(1.9), Inches(2.4), Inches(1.1),
                    "Company", comp["company"][:18], color=color)
        metric_card(slide, cx, Inches(3.1), Inches(2.4), Inches(1.0),
                    "Posting Frequency", comp.get("posting_frequency", "N/A"),
                    sub=f"{comp['total_videos']} total videos", color=color)
        metric_card(slide, cx, Inches(4.2), Inches(2.4), Inches(0.9),
                    "Channel Age", comp.get("channel_since", "N/A")[:7],
                    color=color)

    insight_text = insights.get("posting_insight", "")
    if insight_text:
        add_rect(slide, Inches(0.3), Inches(5.3), Inches(12.7), Inches(1.5), fill=BG_CARD,
                 line_color=ACCENT2, line_width=Pt(1))
        add_text(slide, "💡 Insight", Inches(0.5), Inches(5.38), Inches(4), Inches(0.35),
                 size=Pt(10), bold=True, color=ACCENT2)
        add_text(slide, insight_text, Inches(0.5), Inches(5.65), Inches(12.2), Inches(1.0),
...
def slide_engagement_analysis(prs, report_data):
    ...
    for i, comp in enumerate(companies[:5]):
        cx = Inches(0.3) + i * Inches(2.55)
        color = COMPANY_COLORS[i % len(COMPANY_COLORS)]
        add_rect(slide, cx, Inches(1.9), Inches(2.4), Inches(3.0), fill=BG_CARD, line_color=color, line_width=Pt(1))
        add_text(slide, comp["company"][:18], cx + Inches(0.1), Inches(2.0), Inches(2.2), Inches(0.35),
                 size=Pt(10), bold=True, color=color)
        metrics = [
            ("Avg Views", fmt_num(comp["avg_views"])),
            ("Avg Likes", fmt_num(comp["avg_likes"])),
            ("Avg Comments", fmt_num(comp["avg_comments"])),
            ("Engagement Rate", f"{comp['avg_engagement']}%"),
        ]
        for j, (label, val) in enumerate(metrics):
            my = Inches(2.4) + j * Inches(0.6)
            add_text(slide, label, cx + Inches(0.1), my, Inches(2.2), Inches(0.25),
                     size=Pt(8), color=GRAY)
            add_text(slide, val, cx + Inches(0.1), my + Inches(0.22), Inches(2.2), Inches(0.32),
                     size=Pt(15), bold=True, color=WHITE)

    # Bar chart
    chart_data = ChartData()
    chart_data.categories = [c["company"][:12] for c in companies[:5]]
    chart_data.add_series("Avg Views", [c["avg_views"] for c in companies[:5]])
    chart_data.add_series("Avg Likes", [c["avg_likes"] for c in companies[:5]])
    ...
    if eng_insight:
        add_text(slide, f"💡 {eng_insight}", Inches(0.3), Inches(7.0), Inches(12.7), Inches(0.35),
                 size=Pt(9), color=GRAY, wrap=True)
...
def slide_gap_analysis(prs, report_data):
    ...
    add_text(slide, "Content Gaps", Inches(0.4), Inches(1.9), Inches(6), Inches(0.4),
             size=Pt(13), bold=True, color=ACCENT3)

    for i, gap in enumerate(gaps[:6]):
        gy = Inches(2.35) + i * Inches(0.7)
        add_rect(slide, Inches(0.4), gy, Inches(5.8), Inches(0.6), fill=BG_CARD, line_color=ACCENT3, line_width=Pt(0.5))
        add_rect(slide, Inches(0.4), gy, Inches(0.05), Inches(0.6), fill=ACCENT3)
        add_text(slide, str(gap), Inches(0.6), gy + Inches(0.13), Inches(5.5), Inches(0.35),
                 size=Pt(11), color=WHITE)

    add_text(slide, "Missing Formats", Inches(7.0), Inches(1.9), Inches(6), Inches(0.4),
             size=Pt(13), bold=True, color=ACCENT4)

    for i, fmt in enumerate(missing_formats[:6]):
        fy = Inches(2.35) + i * Inches(0.7)
        add_rect(slide, Inches(7.0), fy, Inches(5.8), Inches(0.6), fill=BG_CARD, line_color=ACCENT4, line_width=Pt(0.5))
        add_rect(slide, Inches(7.0), fy, Inches(0.05), Inches(0.6), fill=ACCENT4)
        add_text(slide, str(fmt), Inches(7.2), fy + Inches(0.13), Inches(5.5), Inches(0.35),
                 size=Pt(11), color=WHITE)
...
def slide_recommendations(prs, report_data):
    ...
    for i, rec in enumerate(recs[:5]):
        ...
        add_rect(slide, rx, ry, Inches(4.1), Inches(2.2), fill=BG_CARD, line_color=color, line_width=Pt(1))
        add_rect(slide, rx, ry, Inches(4.1), Inches(0.4), fill=color)
        num = f"0{i+1}"
        add_text(slide, num, rx + Inches(0.12), ry + Inches(0.05), Inches(0.5), Inches(0.32),
                 size=Pt(14), bold=True, color=WHITE)

        title = rec.get("title", f"Recommendation {i+1}")
        detail = rec.get("detail", "")

        add_text(slide, title[:40], rx + Inches(0.12), ry + Inches(0.5), Inches(3.8), Inches(0.45),
                 size=Pt(12), bold=True, color=WHITE)
        add_text(slide, detail[:200], rx + Inches(0.12), ry + Inches(0.95), Inches(3.8), Inches(1.15),
...
def slide_summary_scores(prs, report_data):
    ...
    if rankings:
        ...
        sorted_companies.sort(key=lambda c: rank_map.get(c["company"], 99))
    ...
    for i, comp in enumerate(sorted_companies[:5]):
        ...
        comp_scores = scores.get(comp["company"], {})
        rank_str = f"#{i+1}"
        row_vals = [
            rank_str,
            comp["company"][:18],
        ...
        for j, (val, cx, cw) in enumerate(zip(row_vals, col_x, col_w)):
            ...
            add_text(slide, val, cx + Inches(0.08), row_y + Inches(0.15), cw - Inches(0.1), Inches(0.35),
                     size=Pt(10 if not is_overall else 12),
                     bold=(j <= 1 or is_overall),
                     color=color if j == 1 else (ACCENT2 if is_overall else WHITE),
                     align=PP_ALIGN.CENTER)

    # Bottom rankings strip
    add_rect(slide, Inches(0.3), Inches(6.0), Inches(12.7), Inches(1.1), fill=BG_CARD, line_color=ACCENT, line_width=Pt(1))
    add_text(slide, "Final Rankings", Inches(0.5), Inches(6.08), Inches(3), Inches(0.35),
             size=Pt(10), bold=True, color=ACCENT)
    ranking_text = "   ".join([f"#{j+1} {name}" for j, name in enumerate(rankings[:5])])
    add_text(slide, ranking_text, Inches(0.5), Inches(6.4), Inches(12.2), Inches(0.55),
